...
export default function ReadyForPickup({ slip, theme, onComplete, error }) {
  ...
  return (
    <div className="border-2 p-3 text-xs space-y-2 h-85 flex flex-col">
      <div>
      ...
      <div className="space-y-1">
        <input
          ...
          className="w-full border px-2 py-1 text-xs outline-none h-10 placeholder:text-gray-500"
        />
        {error && <p className="absolute text-red-500 text-[10px]">{error}</p>}
      </div>
      ...
      <div className="flex justify-end mt-auto">
        <button
          onClick={() => {
            ...
            onComplete(slip._id, otp);
          }}
          className="border-2 px-3 py-1 hover:bg-green-800"
        >
          Complete
        </button>
      </div>
